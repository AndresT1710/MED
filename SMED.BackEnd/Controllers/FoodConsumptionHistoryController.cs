...
namespace SMED.BackEnd.Controllers
{
    ...
    public class FoodConsumptionHistoryController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<FoodConsumptionHistoryDTO>>> GetByClinicalHistoryId(int clinicalHistoryId)
        {
            var allRecords = await _repository.GetAllAsync();
            var filtered = allRecords?.Where(x => x.ClinicalHistoryId == clinicalHistoryId).ToList() ?? new List<FoodConsumptionHistoryDTO>();
            return Ok(filtered);
        }
    }
}
